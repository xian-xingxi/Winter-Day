...
function animate() {
    ...
    if (cloud1x - cnv.width > 0) {
        cloud1x = 0;
        cloud1y = Math.random() * 200;
    }

    // Cloud2 Only run after 270 frame
    if (frameCount >= 270) {

        // Update Cloud 2
        cloud2x++; // Move right

        // If cloud2 goes off right side of canvas, teleport to left side at random height
        if (cloud2x - cnv.width > 0) {
            cloud2x = 0;
